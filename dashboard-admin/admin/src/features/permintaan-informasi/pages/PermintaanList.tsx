...
import { DATA, STATUS_STYLE, type PermintaanInformasi } from '../types/permintaan.types';
...
export default function PermintaanList() {
  ...
  const handleExport = () => {
    const headers = ['No. Tiket', 'Nama Pemohon', 'Kategori', 'Status', 'Tanggal'];
    const csvRows = filtered.map(item =>
      `${item.noTiket},${item.nama},${item.kategori},${item.status},${item.tanggal}`
    );
    ...
    <div className="p-6 bg-[#f4f7fb] min-h-screen">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="grid grid-cols-6 bg-gray-50 px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide border-b border-gray-100">
          <div>No. Tiket</div>
          <div className="col-span-1">Nama Pemohon</div>
          <div>Kategori</div>
          <div>Status</div>
          <div>Tanggal</div>
          <div>Aksi</div>
